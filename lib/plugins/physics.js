...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  bot._client.on('position', (packet) => {
    // Check if the bot is in a specific stance and update the height only when necessary
    if (!bot.entity) return

    if (bot.entity.stance === 'standing') {
      bot.entity.height = 1.8 // Default height
    } else if (bot.entity.stance === 'crawling') {
      bot.entity.height = 0.5 // Crawling stance height
    } else if (bot.entity.stance === 'swimming') {
      bot.entity.height = 0.6 // Swimming stance height
    }
