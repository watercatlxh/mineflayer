...
function inject (bot, { version, storageBuilder, hideErrors }) {
  ...
  function addColumn (args) {
    // If there is no bitmap and groundUp is true, stop storing the chunk column
    if (!args.bitMap && args.groundUp) {
      delColumn(args.x, args.z)
      return
    }

    // Try to get the loaded column, if it doesn't exist, create a new Chunk
    ...
    try {
      // Load the column data and consider biomes and light parameters
      ...
      // If an error occurs, log the error and ignore the exception to prevent the program from crashing
      console.warn(`Failed to load chunk column: ${e.message}`)
      bot.emit('error', e)
    }
  }
